...
# Why sudo and chmod ? Should make better fix
cd target
sudo rm -rf .trivy && mkdir -p .trivy && chmod 777 .trivy
...
# .trivyignore: fichier à mettre à la racine du projet, permet de whitelister certaines CVE
TRIVY_CMD="trivy fs --exit-code 1 --severity HIGH,CRITICAL --format template --template "@/usr/local/share/trivy/templates/html.tpl" -o ${WORKDIR}/target/.trivy/trivy.html ${WORKDIR}"
